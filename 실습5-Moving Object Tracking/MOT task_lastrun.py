...
task_startClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1
...
while continueRoutine:
    ...
    tThisFlip = win.getFutureFlipTime(clock=task_startClock)
...
text_3.setText(str(screenWidth)+"\n"+str(screenHeight))
# keep track of which components have finished
trialComponents = [stim_circle, text_3]
...
while continueRoutine:
    ...
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    if abs(circle_x) >= widthRange or abs(circle_y) >= heightRange: # 모니터 가로 경계(왼쪽, 오른쪽)에 부딪히면
        base_angle += 180
        
    circle_x+=cos(base_angle)*citcle_spd
    circle_y+=sin(base_angle)*citcle_spd
